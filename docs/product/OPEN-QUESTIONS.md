...
| Q-T16 | Default locale by country? | Cookie if set; else Vercel `x-vercel-ip-country` — `TR` → Turkish, anything else → English. Missing header (local) → TR. Country-only, no city. | decided | Toygar |
