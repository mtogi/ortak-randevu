...
7. War plan / build gate: `docs/WAR-PLAN.md`. **M0–M4 done.** Site: `https://www.ortakrandevu.com`. **TR default locale** (2026-09-14) + geo first-visit (Q-T16). LICENSE is PolyForm Noncommercial 1.0.0 (source-available, not OSI). Hygiene queue done. Vercel Web Analytics **on**. Speed Insights out. **Phase 0 done** (host copy + [ADR-009](./architecture/ADR/009-calendar-sync.md)). **Next:** calendar foundation + Google read-busy. Never Rotate `AUTH_SECRET`.
